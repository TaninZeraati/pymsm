...
class MultiStateModel:
    ...
    def fit(self, verbose: int = 1) -> None:
        ...
        for state in self.competing_risk_dataset["origin_state"].unique():
            try:
                ...
                print(f"Model fitted at state {state}***********")
            except Exception as e:
                print(f"Error fitting model at state {state}")
                # print(e, state)
                pass
        if verbose >= 1:
    ...
    def _prepare_dataset_for_competing_risks_fit(self) -> DataFrame:
        ...
        for obj in self.dataset:
            # print(f'inside obj, sample_id: {obj.sample_id}, states: {obj.states}')
            ...
            for i, state in enumerate(obj.states):
                transition_row = {}
                time_in_origin = obj.time_at_each_state[i-1]
                time_transition_to_target = time_entry_to_origin + time_in_origin
                target_state = (
                    obj.states[i + 1] if i + 1 < len(obj.states) else RIGHT_CENSORING
                )
                # print(f'origin_state: {origin_state}, target_state: {target_state}, time_entry_to_origin: {time_entry_to_origin}, time_transition_to_target: {time_transition_to_target}')  
                ...
                if (
                    ...
                    covs_entering_origin = self.update_covariates_fn(
                        covs_entering_origin, origin_state, target_state, time_in_origin, obj.sample_id
                    )
    ...
    def _fit_state_specific_model(
        ...
        if state == 1:
            ...
            state_specific_df = state_specific_df.drop(columns_to_exclude, axis=1)
    ...
    def _one_monte_carlo_run(
        ...
        for i in range(0, max_transitions):
            ...
            next_state = self._sample_next_state(
                current_state, sample_covariates, sample_id, current_time
            )
            ...
            time_to_next_state = self._sample_time_to_next_state(
                current_state, next_state, sample_covariates, sample_id, current_time
            )
    ...
    def _sample_next_state(
        ...
        # print(f"in sample next state, current state: {current_state}, possible next states: {possible_next_states}")
        ...
        if current_state == 1:
            ...
            indices_to_keep = [
                i for i, col in enumerate(self.covariate_names)
                if col not in columns_to_exclude
            ]
            
            # Select features using indices
            sample_covariates = sample_covariates[indices_to_keep]
            # print('sample one after execution', current_state, sample_covariates)
       
            # print(f"no remove sample covariates: {sample_covariates}")
        ...
        for state in possible_next_states:
            ...
            # print('state:', state, 'prob:', probabilites[state])
    ...
    def _sample_time_to_next_state(
        ...
        if current_state == 1:
            ...
            # Select features using indices
            sample_covariates = sample_covariates[indices_to_keep]
            # print('sample one after execution time to next', current_state, sample_covariates)
        # else:
            # print(f"no remove sample covariates time to next: {sample_covariates}")
